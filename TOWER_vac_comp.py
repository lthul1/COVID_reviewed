...
Dlist = []
NT = np.sum(N)
tests = np.arange(0.002, 0.1, 0.004)
Nvacs = np.floor(tests*NT)
count = 0
for m in Nvacs:
    print(str(count) + ' out of ' +str(len(tests)))
    print('vac kits: '+str(m))
    # initialize process classes
    vac_proc = vaccine_process3(nc, T, N)
    test_proc = test_process(nc, T, N)
    # create alias for the methods
    vac_fun = vac_proc.shortage
    ...
    vaccine_policies = [vac_policies.prop_policy, vac_policies.risk_DLA_param]
    mv = len(vaccine_policies)
    vac_names = ['PFA', 'DLA_param']
    ...
    vparam_list = [vparams0, vparams6]

    # list of testing policies
    testing_policies = [test_policies.null_policy]
...
plt.title('Vaccine Kits Available versus Performance')
plt.legend(['PFA', 'DLA'])
plt.show()
