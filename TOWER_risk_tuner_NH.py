...
param0 = np.arange(0.1, 2, .1)
param1 = np.arange(0.1, 2, .1)
...
betahat = gen_betas_NH(nc, T, samples, 2)
...
for k in range(parameterlist.shape[0]):
    ...
    for i in range(samples):
        ...
        vparams_eval = [.05,  6, 0, tune_eval[0], tune_eval[1]]
...
dl.save_data(parameterlist, 'NH_folder/params_'+str(vac_tuner)+'_'+str(current_time)+'.obj')
dl.save_data(Fl, 'NH_folder/costs_'+str(vac_tuner)+'_'+str(current_time)+'.obj')
dl.save_data(Fl_mean, 'NH_folder/mcosts_'+str(vac_tuner)+'_'+str(current_time)+'.obj')
dl.save_data(Fl_std, 'NH_folder/scosts_'+str(vac_tuner)+'_'+str(current_time)+'.obj')
